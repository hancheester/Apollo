...
namespace Apollo.Core.Services.Tasks
{
    public partial class TaskManager
    {
        ...
        public void Initialize()
        {
            ...
            var logBuilder = EngineContext.Current.Resolve<ILogBuilder>();
            var logger = logBuilder.CreateLogger(GetType().FullName);
            ...
            foreach (var scheduleTaskGrouped in scheduleTasks.GroupBy(x => x.Seconds))
            {
                ...
                foreach (var scheduleTask in scheduleTaskGrouped)
                {
                    var task = new Task(
                        ...
                        taskService, 
                        logger, 
                        (TaskFrequencyType?)scheduleTask.Frequency,
                        scheduleTask.NextStartUtc);
